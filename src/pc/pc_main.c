...
void produce_one_dummy_frame(void (*callback)(), u8 clearColorR, u8 clearColorG, u8 clearColorB) {
    ...
    if (remaining > 0) {
#ifdef TARGET_WEB
        // Skip gWindowApi->delay on web — it uses emscripten_sleep which causes
...
static void web_auto_network(void) {
    ...
    if (web_peer_waiting) {
        ...
        int roleKnown = EM_ASM_INT({
            // PartyKit assigns the host/client role when the room socket opens.
            if (!PeerNetwork.isConnected()) return 0;
            return PeerNetwork.isHost ? 1 : 2;
        });
